...
namespace DataService
{
    public class PLCGroup : IGroup
    {
        ...
        public PLCGroup(short id, string name, int updateRate, bool active, IPLCDriver plcReader)
        {
            ...
            this._server = _plcReader.Parent;//用驱动带的DataServer 来初始化 Group
        ...
        /// <summary>
        /// 通过传入的从数据库中 读取的 TagMetaData数据(DeviceAddress 为string) 来添加为 Itag(DeviceAddress 已为结构体)，这个函数已经根据GroupID 
        ...
        /// <summary>
        /// 该函数组要的目的是为了跟新Itag在内存缓冲区的索引地址（这是一个需要计算的值，不能直接赋值），其次是为了判别 累加的 ITag长度不要超过PDU的长度，在我们的程序里面PDU可以设置无限大
        /// 问题是我们在同一段字节中定义了不同的 变量，会如何影响这个函数的执行？
        /// </summary>
        protected void UpdatePDUArea()
        {
            int count = _items.Count;//从 IList<TagMetaData> items 来 的 ITag 列表
            if (count > 0) 
            {
                ...
                if (count > 1)
                {
                    ...
                    int cacheIndexStart = 0;//新的PDU在缓冲区开始得索引号
                    int startIndex = 0;//PDUArea 开始的在Itag列表里的索引号
                    ...
                    for (int j = 1, i = 1; i < count; i++, j++)
                    {
                        ...
                        int offset1 = _cacheReader.GetOffset(tagAddress, segmentStart);//第一个 第二个 两个起始(Start)位置相减，得到 偏移量 offset1,起始每次都是后面的 和 第一个Itag地址，算偏移量
                        if (offset1 > (_plcReader.PDU - tagAddress.DataSize) / bitCount)//？？代表的估计是最后一个Itag
                        {
                            segmentEnd = _items[i - 1].Address;//往前面倒退一个Itag 地址，因为自己超过了 PDU 的大小
                            int len = _cacheReader.GetOffset(segmentEnd, segmentStart);//算出组后一个地址 距离第一个的偏移量
                            len += segmentEnd.DataSize <= bitCount ? 1 : segmentEnd.DataSize / bitCount;//最后一个的偏移量 加上自己的长度，
                            tagAddress.CacheIndex = (ushort)(cacheIndexStart + len);//更新在缓存中的字节索引
                            _items[i].Address = tagAddress;//这是后面一个Itag的地址
                            ...
                            cacheIndexStart = cacheLength;//新的PDU 开始的字节索引地址
                            segmentStart = tagAddress;//更新数据片段的起始地址，新的起始地址
                        }
                        else
                        {
                            tagAddress.CacheIndex = (ushort)(cacheIndexStart + offset1);//就是更新Itag在内存缓存中的字节索引号,所以这个是和内存关联起来的
                            _items[i].Address = tagAddress;
                        }
                        if (i == count - 1)//此处代表到了最后一个ITag 地址
                        {
                            segmentEnd = _items[i].Address;
                            int segmentLength = _cacheReader.GetOffset(segmentEnd, segmentStart);//最后一个距离第一个Itag的地址偏移量，每次都是算第一个的
                            if (segmentLength > (_plcReader.PDU - segmentEnd.DataSize) / bitCount)//真的回运行到这个 if 里面去吗?，在上一个if else
                            {
        ...
        /// <summary>
        /// 主通信流程：Step 1：开启定时器，从内存中读取数据进行比较，看是否更新状态
        /// </summary>
    ...
    public struct PDUArea
    {
        ...
        public int Count;//代表当前PDU地址块所包含的Itag的个数
                        ...
                        if (addr.VarType == DataType.BOOL)
                        {
                            ...
                            if (tmp != 0)
                            {
                                while (addr.Start == next.Start)
                                {
                                    ...
                                    if (++index < count)
                                    {
                                        if (_items[index].Address.VarType == DataType.BOOL)
                                        {
                                            next = _items[index].Address;
                                        }
                                        else
                            ...
                            {
                                while ((addr.Start == next.Start) && (++index < count) && (addr.VarType == DataType.BOOL))
                                {
                                    next = _items[index].Address;
                                }
                            }
