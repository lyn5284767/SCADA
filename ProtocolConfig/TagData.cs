...
namespace ProtocolConfig
{
    ...
    public class TagData : IComparable<TagData>
    {
        ...
        public int CompareTo(TagData other)
        {
            ...
            //int cmp = this._groupId.CompareTo(other._groupId);
            //return cmp == 0 ? this.GetSequenceByfloat(other) : cmp;
        }

        public int GetSequenceByfloat(TagData other)
        {
            int index = this.Address.IndexOf('.');
            float fSequence = -1;
            if (index > 0)
            {
                fSequence = float.Parse(this.Address);
            }
            else
            {
                fSequence = int.Parse(this.Address.Substring(1));
            }


            index = other.Address.IndexOf('.');
            float fSequenceOther = -1;
            if (index > 0)
            {
                fSequenceOther = float.Parse(other.Address);
            }
            else
            {
                fSequenceOther = int.Parse(other.Address.Substring(1));
            }

           return  fSequence == fSequenceOther ? 0 : fSequence > fSequenceOther ? 1 : -1;
        }
    }
    ...
    public class TagDataReader : IDataReader
    {
        ...
        {
            DataTable table = new DataTable("Protocol");
